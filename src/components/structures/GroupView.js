...
class CategoryRoomList extends React.Component {
    ...
    onAddRoomsToSummaryClicked = (ev) => {
        ...
        Modal.createTrackedDialog(
            "Add Rooms to Group Summary",
            "",
            AddressPickerDialog,
            {
                title: _t("Add rooms to the community summary"),
                description: _t(
                    "Which rooms would you like to add to this summary?"
                ),
                ...
                onFinished: (success, addrs) => {
                    ...
                    allSettled(
                        addrs.map((addr) => {
                            return GroupStore.addRoomToGroupSummary(
                                this.props.groupId,
                                addr.address
                            ).catch(() => {
    ...
    render() {
        ...
        return (
            <div className="mx_GroupView_featuredThings_container">
                {catHeader}
                {roomNodes}
                {addButton}
            </div>
...
class FeaturedRoom extends React.Component {
        ...
        GroupStore.removeRoomFromGroupSummary(
            ...
            Modal.createTrackedDialog(
                ...
                ErrorDialog,
                {
                    ...
                    description: _t(
                        "The room '%(roomName)s' could not be removed from the summary.",
                        { roomName }
                    ),
                }
            );
    ...
    render() {
        ...
        let permalink = null;
        if (
            this.props.summaryInfo.profile &&
            this.props.summaryInfo.profile.canonical_alias
        ) {
            permalink = makeGroupPermalink(
                this.props.summaryInfo.profile.canonical_alias
            );
        }
        ...
        Modal.createTrackedDialog(
            ...
            AddressPickerDialog,
            {
                ...
                onFinished: (success, addrs) => {
                    ...
                    allSettled(
                        addrs.map((addr) => {
                            return GroupStore.addUserToGroupSummary(
                                addr.address
                            ).catch(() => {
                                errorList.push(addr.address);
                            });
...
class FeaturedUser extends React.Component {
    ...
    onDeleteClicked = (e) => {
        ...
        GroupStore.removeUserFromGroupSummary(
            ...
            const displayName =
                this.props.summaryInfo.displayname ||
                this.props.summaryInfo.user_id;
            const ErrorDialog = sdk.getComponent("dialogs.ErrorDialog");
            Modal.createTrackedDialog(
    ...
    render() {
        ...
        const httpUrl = MatrixClientPeg.get().mxcUrlToHttp(
            this.props.summaryInfo.avatar_url,
            64,
            64
        );
...
export default class GroupView extends React.Component {
    ...
    componentDidMount() {
        ...
        this._rightPanelStoreToken = RightPanelStore.getSharedInstance().addListener(
            this._onRightPanelStoreUpdate
        );
    }
    ...
    _initGroupStore(groupId, firstInit) {
        ...
        GroupStore.registerListener(
            groupId,
            this.onGroupStoreUpdated.bind(this, firstInit)
        );
        ...
        GroupStore.on("error", (err, errorGroupId, stateKey) => {
            ...
            if (
                err.errcode === "M_GUEST_ACCESS_FORBIDDEN" &&
                !willDoOnboarding
            ) {
                dis.dispatch({
    ...
    onGroupStoreUpdated = (firstInit) => {
        ...
        this.setState({
            summary,
            summaryLoading: !GroupStore.isStateReady(
                this.props.groupId,
            ...
            groupRoomsLoading: !GroupStore.isStateReady(
                this.props.groupId,
    ...
    _onEditClick = () => {
        this.setState({
            ...
            joinableForm: {
                policyType: this.state.summary.profile.is_openly_joinable
    ...
    _onShareClick = () => {
        ...
        Modal.createTrackedDialog("share community dialog", "", ShareDialog, {
            target:
                this._matrixClient.getGroup(this.props.groupId) ||
                new Group(this.props.groupId),
        });
    };
        ...
        savePromise
            .then((result) => {
                ...
                if (this.state.avatarChanged) {
                    ...
                    FlairStore.refreshGroupProfile(
                        this._matrixClient,
                        this.props.groupId
                    );
                }
    ...
    async _saveGroup() {
        await this._matrixClient.setGroupProfile(
            this.props.groupId,
    ...
    _onAcceptInviteClick = async () => {
        ...
        GroupStore.acceptGroupInvite(this.props.groupId)
            .then(() => {
    ...
    _onRejectInviteClick = async () => {
        ...
        GroupStore.leaveGroup(this.props.groupId)
            .then(() => {
                // don't reset membershipBusy here: wait for the membership change to come down the sync
            })
            .catch((e) => {
    ...
    _onJoinClick = async () => {
        ...
        GroupStore.joinGroup(this.props.groupId)
            .then(() => {
                // don't reset membershipBusy here: wait for the membership change to come down the sync
            })
            .catch((e) => {
        ...
        if (this.state.isUserPrivileged) {
            warnings.push(
                <span className="warning">
                    ...
                    {_t(
                        "You are an administrator of this community. You will not be " +
    ...
    _getGroupSection() {
        const groupSettingsSectionClasses = classnames({
            ...
            mx_GroupView_group_disabled:
                this.state.editing && !this.state.isUserPrivileged,
        });

        const header = this.state.editing ? (
            <h2> {_t("Community Settings")} </h2>
        ) : (
            <div />
        );
        ...
        if (hostingSignupLink && this.state.isUserPrivileged) {
            hostingSignup = (
                <div className="mx_GroupView_hostingSignup">
                    {_t(
        ...
        return (
            <div className={groupSettingsSectionClasses}>
                {header}
                {hostingSignup}
                {changeDelayWarning}
    ...
    _getFeaturedRoomsNode() {
        ...
        return (
            <div className="mx_GroupView_featuredThings">
    ...
    _getMembershipSection() {
        ...
        if (group && group.myMembership === "invite") {
            if (this.state.membershipBusy || this.state.inviterProfileBusy) {
                return (
                    <div className="mx_GroupView_membershipSection">
                        <Spinner />
                    </div>
                );
            }
            const httpInviterAvatar = this.state.inviterProfile
                ? this._matrixClient.mxcUrlToHttp(
                      this.state.inviterProfile.avatarUrl,
                      36,
                      36
                  )
                : null;
            ...
            if (this.state.inviterProfile) {
                inviterName =
                    this.state.inviterProfile.displayName || inviter.userId;
            }
            return (
                <div className="mx_GroupView_membershipSection mx_GroupView_membershipSection_invited">
                    <div className="mx_GroupView_membershipSubSection">
                        <div className="mx_GroupView_membershipSection_description">
                            ...
                            />
                            {_t(
                                "%(inviter)s has invited you to join this community",
                                {
        ...
        if (
            (!group || group.myMembership === "leave") &&
            this.state.summary &&
            ...
            membershipContainerExtraClasses =
                "mx_GroupView_membershipSection_leave";
        } else if (
            group &&
            ...
            membershipContainerExtraClasses =
                "mx_GroupView_membershipSection_joined";
        } else {
            return null;
        }

        const membershipButtonClasses = classnames(
            ["mx_RoomHeader_textButton", "mx_GroupView_textButton"],
            membershipButtonExtraClasses
        );
        ...
        return (
            <div className={membershipContainerClasses}>
                <div className="mx_GroupView_membershipSubSection">
                ...
                <div>
                    <label>
                        <input
                            ...
                            checked={
                                this.state.joinableForm.policyType ===
                                GROUP_JOINPOLICY_INVITE
                            }
    ...
    render() {
        ...
        if (
            (this.state.summaryLoading && this.state.error === null) ||
            this.state.saving
        ) {
            return <Spinner />;
            ...
            if (this.state.editing && this.state.isUserPrivileged) {
                ...
                const onGroupHeaderItemClick = this.state.isUserMember
                    ? this._onEditClick
                    : null;
                const groupAvatarUrl = summary.profile
                ...
                if (summary.profile && summary.profile.name) {
                    nameNode = (
                        <div onClick={onGroupHeaderItemClick}>
                    ...
                    nameNode = (
                        <span onClick={onGroupHeaderItemClick}>
                            {this.props.groupId}
                        </span>
                    );
                }
            ...
            const rightPanel = this.state.showRightPanel ? (
            ...
            if (this.state.error.httpStatus === 404) {
                return (
                    <div className="mx_GroupView_error">
                        {_t("Community %(groupId)s not found", {
                            groupId: this.props.groupId,
                        })}
                    </div>
                );
