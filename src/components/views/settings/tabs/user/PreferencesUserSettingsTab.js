...
export default class PreferencesUserSettingsTab extends React.Component {
    static ROOM_LIST_SETTINGS = ["breadcrumbs"];
    ...
    constructor() {
        ...
        this.state = {
            ...
            autocompleteDelay: SettingsStore.getValueAt(
                SettingLevel.DEVICE,
                "autocompleteDelay"
            ).toString(10),
    ...
    _onAutoLaunchChange = (checked) => {
        PlatformPeg.get()
            ...
            .then(() => this.setState({ autoLaunch: checked }));
    };

    _onAlwaysShowMenuBarChange = (checked) => {
        PlatformPeg.get()
            .setAutoHideMenuBarEnabled(!checked)
            .then(() => this.setState({ alwaysShowMenuBar: checked }));
    };

    _onMinimizeToTrayChange = (checked) => {
        PlatformPeg.get()
            .setMinimizeToTrayEnabled(checked)
            .then(() => this.setState({ minimizeToTray: checked }));
    };
    ...
    _renderGroup(settingIds) {
        ...
        return settingIds.map((i) => (
            <SettingsFlag key={i} name={i} level={SettingLevel.ACCOUNT} />
        ));
    }
        ...
        return (
            <div className="mx_SettingsTab mx_PreferencesUserSettingsTab">
                ...
                <div className="mx_SettingsTab_section">
                    ...
                    {this._renderGroup(
                        PreferencesUserSettingsTab.ROOM_LIST_SETTINGS
                    )}
                </div>
                ...
                <div className="mx_SettingsTab_section">
                    ...
                    {this._renderGroup(
                        PreferencesUserSettingsTab.TIMELINE_SETTINGS
                    )}
                </div>
                ...
                <div className="mx_SettingsTab_section">
                    ...
                    {this._renderGroup(
                        PreferencesUserSettingsTab.ADVANCED_SETTINGS
                    )}
                    {minimizeToTrayOption}
                    {autoHideMenuOption}
                    {autoLaunchOption}
